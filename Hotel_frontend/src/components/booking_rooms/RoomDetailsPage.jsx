...
const RoomDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const userProfile = await ApiServices.getUserProfile();
    ...
    const handleConfirmBooking = async () => {
        ...
        const toltalDays = Math.round(Math.abs((endDate - startDay) / oneDay)) + 1;

        const totalGuests = numAdults + numChildren;

        const roomPricePerNight = roomDetails.roomPrice;
        const totalPrice = roomPricePerNight * toltalDays;
    ...
    const acceptBooking = async () => {
        try {
            ...
            const formattedCheckOutDate = new Date(endDate.getTime() - (endDate.getTimezoneOffset() * 6000)).toISOString().split('T')[0];
            ...
            const response = await ApiServices.bookRoom(roomId, userId, booking);
