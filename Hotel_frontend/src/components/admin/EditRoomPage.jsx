...
const EditRoomPage = () => {
    ...
    const handleFileChange = (e) => {
        const slectedFile = e.target.files[0];
        if (slectedFile) {
            setFile(slectedFile);
            setPreview(URL.createObjectURL(slectedFile));
        } else {
            setFile(null);
            setPreview(null);
        }
    ...
    const handleUpdate = async () => {
        try {
            ...
            const result = await ApiServices.updateRoom(roomId, formData);
    ...
    const handleDelete = async () => {
        if (window.confirm('Do you want to delete this room?')) {
            try {
                const result = await ApiServices.deleteUserById(roomId);
                if (result.statusCode === 200) {
